...

{
    ...
    like($r10, qr/\|  1 \|/, 'render: single-digit labels are right-justified in width-2');
}

{
    # 10x10 grid, 100 monominos -> labels 1..100, width=3
    my @mono100;
    for my $r (0..9) { for my $c (0..9) { push @mono100, [[$r,$c]] } }
    my $r100 = Polyomino::Renderer::render(\@mono100, 10, 10);
    like($r100, qr/\| 100 \|/, 'render: 100 pieces uses width-3 labels');
    like($r100, qr/\|   1 \|/, 'render: single-digit labels right-justified in width-3');
    unlike($r100, qr/\?/, 'render: no unassigned cells in 10x10 monomino grid');
}
