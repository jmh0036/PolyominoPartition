...
# ── suggest_pieces ────────────────────────────────────────────────────────
{
    my $pieces = Polyomino::Tiler->suggest_pieces(
        n => 4, m => 4, must => [3, 5], fill => 4,
    );
    my $sum = 0; $sum += $_ for @$pieces;
    is($sum, 16, 'suggest_pieces: result sums to n*m');
    is(scalar @$pieces, 4, 'suggest_pieces: correct piece count');
    ok((grep { $_ == 3 } @$pieces), 'suggest_pieces: includes must piece of size 3');
    ok((grep { $_ == 5 } @$pieces), 'suggest_pieces: includes must piece of size 5');
}

{
    eval {
        Polyomino::Tiler->suggest_pieces(n => 3, must => [4], fill => 2);
    };
    like($@, qr/not divisible/, 'suggest_pieces: dies when remainder not divisible by fill');
}

{
    eval {
        Polyomino::Tiler->suggest_pieces(n => 2, m => 2, must => [3, 5], fill => 1);
    };
    ...
    for my $piece (@$solution) {
        ...
        for my $cell (@$piece) {
            ...
            return (0, "cell $key out of bounds")
                if $cell->[0] < 0 || $cell->[0] >= $n
                || $cell->[1] < 0 || $cell->[1] >= $m;
        }
    }
    return (0, "not all cells covered") unless scalar keys %coverage == $n * $m;
    for my $k (keys %expected) {
        return (0, "wrong count of size-$k: got " . ($got_sizes{$k}//0) . " want $expected{$k}")
...

{
    ...
    my @s24 = $t24->solve();
    my @s42 = $t42->solve();
    is(scalar @s24, scalar @s42, '[2,4] and [4,2] give same number of solutions');
}
